class HomeNode {
  ...
  constructor(id: string, label: string, properties: any, nodeType: string) {
    ...
    switch (nodeType) {
      case "room":
        image = "https://image.flaticon.com/icons/svg/489/489870.svg";
        break;
      case "window":
        image = "https://image.flaticon.com/icons/svg/544/544081.svg";
        break;
      case "door":
        image = "https://image.flaticon.com/icons/svg/515/515094.svg";
        break;
    }
    this.descriptionCfg = {
      positions: "bottom",
  ...
  floor.rooms.forEach((room: any) => {
    ...
    room.windows.forEach((window: any) => {
      if (!nodes.some(node => node.id === window.name)) {
        nodes.push(
          new HomeNode(window.name, window.name, window.properties, "window")
        );
