...
object ConfigDsl {
  ...
  def time_now(): JsonPropertyFactory[Long] = JsonPropertyFactory.safeDynamic("time", () => System.currentTimeMillis)
  ...
  def mqtt_bool(name: String, topic: String, caseTrue: String, caseFalse: String)(implicit brokerConfig: BrokerConfig): JsonPropertyFactory[Boolean] =
  ...
  implicit class JsonPropertyFactoryImplicit[T: JsonFormat](source: Source[Try[T], _]) {
    def asJsonProperty(name: String): JsonPropertyFactory[T] =
      JsonPropertyFactory.fromStream(name, () => source)
  }
...
object Test extends App {
  ...
  door(hallway -> external).withProperties(
    ...
    } asJsonProperty "Scrivania",

    mqtt_bool("Luce letto", "stat/shelly25_1/POWER2", "ON", "OFF"),
